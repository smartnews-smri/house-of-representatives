...
def parse_kaiji(kaiji):
  ...
  def parse_keika_all(kaiji, result):
    ...
    def parse_keika(row):
      ...
      for i, td in enumerate(tds):
        ...
        if td.text == "議案提出者一覧" or td.text == "議案提出の賛成者":
          komokus.append(td.text)
          naiyos.append(tds[i + 1].text)
          skip_index = i + 1  # Append both Komoku and Naiyo, then skip the next
        else:
  ...
  def update_gian_file(kaiji, result):
    ...
    gian_all_raw = get_csv(DIR_DATA + "gian.csv")
    ...
    save_file(DIR_DATA + "gian.csv", gian_all)
    save_file(DIR_DATA + "gian.json", gian_all)
...
def update_gian_summary():
  gian_all = get_csv(DIR_DATA + "gian.csv")
  gian_sum = []
  ...
  for a_row in gian_all[1:]:
    ...
    if s_index >= 0:
      gian_sum[s_index][4] = a_row[0]   # 掲載回次
      gian_sum[s_index][5] = a_row[6]   # 審議状況
      gian_sum[s_index][6] = a_row[12]  # 議案提出者
      gian_sum[s_index][7] = a_row[13]  # 議案提出会派
      gian_sum[s_index][8] = a_row[-2]  # 議案提出者一覧
      gian_sum[s_index][9] = a_row[-1]  # 議案提出の賛成者
      gian_sum[s_index][10].append(appendrow)
    else:
      newrow = [
        a_row[11],    # 議案種類
        a_row[3],     # 提出回次
